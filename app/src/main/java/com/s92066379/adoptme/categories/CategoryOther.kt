...
import com.s92066379.adoptme.databinding.ActivityCategoryOtherBinding
import dagger.hilt.android.AndroidEntryPoint

@AndroidEntryPoint
class CategoryOther : AppCompatActivity() {

    private lateinit var binding: ActivityCategoryOtherBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityCategoryOtherBinding.inflate(layoutInflater)
        setContentView(binding.root)

        recyclerView = findViewById(R.id.recyclerOther)
        recyclerView.layoutManager = LinearLayoutManager(this)

        adapter = ListingsAdapter(listings, listingIds)
        recyclerView.adapter = adapter

        binding.btnBackOther.setOnClickListener { onBackPressed() }
